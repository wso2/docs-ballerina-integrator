...
service /healthcare on ln {
    ...
    resource function get payments() returns Payment[] {
        lock {
            return <readonly>from Payment payment in payments
                select payment;
        }
    }
...
service on ln {
    resource function post [string hospital]/categories/[string category]/reserve(AppointmentRequest & readonly appointmentRequest)
        ...
        final string patientName = appointmentRequest.patient.name;
        ...
        lock {
            string ssn = appointmentRequest.patient.ssn;
            if !patientRecords.hasKey(ssn) {
                patientRecords[ssn] = {patientName};
            }
        }
        return appointment;
    }
    ...
    resource function get [string hospital]/categories/appointments/[int appointment_id]/fee()
        ...
        lock {
            if appointments.hasKey(appointment_id) {
                Appointment {doctor: {name: doctorName, fee}, patientName} = appointments.get(appointment_id);
                return {
                    patientName,
                    doctorName,
